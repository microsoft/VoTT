...
fdescribe('E2E - Export', () => {
    ...
    describe('Exporting Videos', () => {
        ...
        it('clicking "Export" button starts the export and displays load spinner', (done) => {
            app.client.windowHandles()
                .then(result => result.value[result.value.length - 1])
                .then(handle => app.client.window(handle))
                .then(() => app.client.pause(2000))
                ...
                .then(() => {
                    ...
                    remove(path.join(__dirname, 'sample_data','sample_images_output'), done);
                    // let expectedPath = path.join(__dirname, 'sample_data','sample_images_output','pascal_label_map.pbtxt')
    ...
    // describe('Exporting Images to TFRecords', () => {
    //     let tagCount = 0;
    //     let originalImageValue = null;
    ...
    //             .then(() => app.client.pause(2000))
    //             // .then(() => {
    //             //     let expectedPath = path.join(__dirname, 'sample_data','sample_images_output','nike1.png.tfrecord')
    //             //     fs.readFile(expectedPath, 'utf8', (err, data) => {
    //             //         if (err) {
    //             //             throw new Error(err);
    //             //         }
    ...
    //             // })
    //             .then(() => {
    //                 let testRecord = readRecord(path.join(__dirname, 'sample_data','sample_images_output','nike1.png.tfrecord'));
    //                 expect(decode_Uint8(testRecord.features.feature['image/object/class/text'].bytesList.value[i])).toEqual('nike');
    //                 remove(path.join(__dirname, 'sample_data','sample_images_output'), done);
    //                 })
    //             .then(() => app.browserWindow.isVisible())
    //             .then(result => expect(result).toBeFalsy())
    ...
    // describe('Exporting Videos to TFRecords', () => {
    //     let tagCount = 0;
    //     let originalImageValue = null;
