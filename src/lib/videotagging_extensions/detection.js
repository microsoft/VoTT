        ...
        self.detectionAlgorithmManager.initExporter(method, exportPath, self.videotagging.inputtagsarray,
            ...
            (err, exporter) => {
                ...
                if (exportUntil === "tagged") {
                    isLastFrame = function (frameId) {
                        let taggedFrames = Object.keys(self.videotagging.frames).filter(key => self.videotagging.frames[key].length).reduce((res, key) => (res[key] = self.videotagging.frames[key], res), {}); // eslint-disable-line
